...
describe('AzureAppServiceDetector', () => {
    ...
    it('should get linux attributes', () => {
        sinon.stub(azureVmDetector, 'getAzureVmMetadata').resolves(linuxTestResponse);
        const attributes = azureVmDetector.detect().attributes;
        for (let i = 0; i < Object.keys(linuxAttributes).length; i++) { 
            const key = Object.keys(linuxAttributes)[i];
            assert.strictEqual(attributes[key], linuxAttributes[key]);
        }
    });

    it('should get windows attributes', () => {
        sinon.stub(azureVmDetector, 'getAzureVmMetadata').resolves(windowsTestResponse);
        const attributes = azureVmDetector.detect().attributes;
        for (let i = 0; i < Object.keys(windowsAttributes).length; i++) { 
            const key = Object.keys(windowsAttributes)[i];
