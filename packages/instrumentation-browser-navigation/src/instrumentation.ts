...
export const URL_FULL_ATTRIBUTE = 'url.full';
export const NAV_SAME_DOCUMENT = 'browser.navigation.same_document';
export const NAV_HASH_CHANGE = 'browser.navigation.hash_change';
export const NAV_TYPE = 'browser.navigation.type';
...
export class BrowserNavigationInstrumentation extends InstrumentationBase<BrowserNavigationInstrumentationConfig> {
  ...
  private _onHardNavigation() {
    const navLogRecord: LogRecord = {
      ...
      attributes: {
        [URL_FULL_ATTRIBUTE]: this._sanitizeUrl(document.documentURI),
  ...
  private _isHashChange(fromUrl: string, toUrl: string): boolean {
    try {
      ...
      return (fromUrl.split('#')[0] === toUrl.split('#')[0]) &&
        fromUrl.includes('#') !== toUrl.includes('#');
    }
  }
  ...
  private _sanitizeUrl(url: string): string {
    try {
      ...
      // If URL parsing fails, return original but try to redact obvious credentials
