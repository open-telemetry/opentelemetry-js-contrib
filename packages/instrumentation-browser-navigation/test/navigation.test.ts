  ...
  describe('export navigation LogRecord', () => {
    it("should export LogRecord for browser.navigation when 'DOMContentLoaded' event is fired", done => {
      ...
      setTimeout(() => {
        ...
        const navLogRecord = exporter.getFinishedLogRecords()[0] as ReadableLogRecord;
        ...
        assert.deepEqual(navLogRecord.attributes, {
          [URL_FULL_ATTRIBUTE]: expectedUrl,
          [NAV_SAME_DOCUMENT]: false,
          [NAV_HASH_CHANGE]: false,
        });
        done();
      });
    });
    ...
    it('should export LogRecord for browser.navigation with type push when history.pushState() is called', done => {
      ...
      const navLogRecord = exporter.getFinishedLogRecords()[0] as any as ReadableLogRecord;
      assert.strictEqual(navLogRecord.eventName, EVENT_NAME);
      // URL should be sanitized - check it matches current location
      const actualUrl = (navLogRecord.attributes as any)[URL_FULL_ATTRIBUTE];
      assert.ok(actualUrl.includes(window.location.pathname), 
        `Expected URL to contain pathname ${window.location.pathname}, got ${actualUrl}`);
      assert.strictEqual(
        (navLogRecord.attributes as any)[NAV_SAME_DOCUMENT],
        true
      );
      assert.strictEqual(
        (navLogRecord.attributes as any)[NAV_HASH_CHANGE],
        false
      );
      assert.strictEqual(
        (navLogRecord.attributes as any)[NAV_TYPE],
        'push'
      );
      assert.strictEqual(
    ...
    it('should not export LogRecord for browser.navigation if the URL is not changed.', done => {
      ...
      const navLogRecord2 = exporter.getFinishedLogRecords()[0] as any as ReadableLogRecord;
      assert.strictEqual(navLogRecord2.eventName, EVENT_NAME);
      // URL should be sanitized - check it matches current location
      const actualUrl2 = (navLogRecord2.attributes as any)[URL_FULL_ATTRIBUTE];
      assert.ok(actualUrl2.includes(window.location.pathname), 
        `Expected URL to contain pathname ${window.location.pathname}, got ${actualUrl2}`);
      assert.strictEqual(
        (navLogRecord2.attributes as any)[NAV_SAME_DOCUMENT],
        true
      );
      assert.strictEqual(
        (navLogRecord2.attributes as any)[NAV_HASH_CHANGE],
        false
      );
    ...
    it('should export LogRecord with hash_change=true when location.hash changes', done => {
      ...
      exporter.reset();
      instrumentation.oldUrl = location.href;
      ...
      const checkForHashChangeRecord = () => {
        ...
        const hashChangeRecord = records.find(record => 
          (record.attributes as any)[NAV_HASH_CHANGE] === true
        );
        
        if (hashChangeRecord) {
          assert.strictEqual(hashChangeRecord.eventName, EVENT_NAME);
          assert.strictEqual(
            (hashChangeRecord.attributes as any)[NAV_SAME_DOCUMENT],
            true
          );
          assert.strictEqual(
            (hashChangeRecord.attributes as any)[NAV_HASH_CHANGE],
            true
          );
          // Accept either 'push' or 'traverse' as browsers may vary
          const navType = (hashChangeRecord.attributes as any)[NAV_TYPE];
    ...
    it('should export LogRecord when Navigation API navigate event is fired (if available)', done => {
      ...
      if (!(window as any).navigation) {
        console.log('Navigation API not available in test environment, skipping test');
        done();
        return;
      }
      ...
      navigateHandler = (event: any) => {
        ...
        setTimeout(() => {
          ...
          if (records.length >= 1) {
            const navLogRecord = records.slice(-1)[0] as ReadableLogRecord;
            assert.strictEqual(navLogRecord.eventName, EVENT_NAME);
            assert.strictEqual((navLogRecord.attributes as any)[NAV_SAME_DOCUMENT], 
              event.destination?.sameDocument ?? true);
            assert.strictEqual((navLogRecord.attributes as any)[NAV_HASH_CHANGE], 
      ...
      try {
        ...
        if (navigation.navigate) {
          ...
          navigation.addEventListener('navigate', interceptHandler, { once: true });
          ...
          setTimeout(() => {
            ...
            if (records.length >= 1) {
              const navLogRecord = records.slice(-1)[0] as ReadableLogRecord;
              assert.strictEqual(navLogRecord.eventName, EVENT_NAME);
              assert.strictEqual((navLogRecord.attributes as any)[NAV_SAME_DOCUMENT], true);
              cleanup();
              done();
            }
          }, 50);
        ...
        if (event.hashChange) {
          setTimeout(() => {
            ...
            if (records.length >= 1) {
              ...
              assert.strictEqual(navLogRecord.eventName, EVENT_NAME);
              assert.strictEqual((navLogRecord.attributes as any)[NAV_SAME_DOCUMENT], true);
              assert.strictEqual((navLogRecord.attributes as any)[NAV_HASH_CHANGE], true);
              assert.strictEqual((navLogRecord.attributes as any)[NAV_TYPE], 'push');
              cleanup();
              done();
            }
          }, 10);
    ...
    it('should sanitize URLs with credentials', done => {
      ...
      const testUrl = 'https://user:password@example.com/path?api_key=secret123&normal=value';
      const sanitized = (instrumentation as any)._sanitizeUrl(testUrl);
      
      assert.ok(sanitized.includes('REDACTED:REDACTED@'), 'Should redact credentials');
      assert.ok(sanitized.includes('api_key=REDACTED'), 'Should redact sensitive query params');
      assert.ok(sanitized.includes('normal=value'), 'Should preserve normal query params');
      done();
    });
    ...
    it('should work with Navigation API disabled', done => {
      ...
      setTimeout(() => {
        ...
        assert.ok(records.length >= 1, 'Should have at least one navigation record');
        ...
        assert.strictEqual((testRecord.attributes as any)[NAV_TYPE], 'push');
        assert.strictEqual((testRecord.attributes as any)[NAV_SAME_DOCUMENT], true);
    ...
    it('should not attach Navigation API listeners when disabled', done => {
      ...
      const navigationSpy = sandbox.spy((window as any).navigation, 'addEventListener');
      ...
      const navigateListenerCalls = navigationSpy.getCalls().filter(call => 
        call.args[0] === 'navigate'
      );
      assert.strictEqual(navigateListenerCalls.length, 0, 
