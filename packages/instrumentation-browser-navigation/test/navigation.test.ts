...
describe('Browser Navigation Instrumentation', () => {
  ...
  describe('export navigation LogRecord', () => {
    ...
    it('should not export LogRecord for browser.navigation if the URL is not changed.', done => {
      ...
      assert.strictEqual(
        (navLogRecord2.attributes as any)[ATTR_BROWSER_NAVIGATION_SAME_DOCUMENT],
        true
      );
      assert.strictEqual(
    ...
    it('should export LogRecord with hash_change=true when location.hash changes', done => {
      ...
      const checkForHashChangeRecord = () => {
        ...
        const hashChangeRecord = records.find(record => 
          (record.attributes as any)[ATTR_BROWSER_NAVIGATION_HASH_CHANGE] === true
        );
        
        if (hashChangeRecord) {
          assert.strictEqual(hashChangeRecord.eventName, EVENT_NAME);
          assert.strictEqual(
            (hashChangeRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_SAME_DOCUMENT],
            true
          );
          assert.strictEqual(
            (hashChangeRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_HASH_CHANGE],
            true
          );
          // Accept either 'push' or 'traverse' as browsers may vary
          const navType = (hashChangeRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_HASH_TYPE];
      ...
      navigateHandler = (event: any) => {
        ...
        setTimeout(() => {
          ...
          if (records.length >= 1) {
            const navLogRecord = records.slice(-1)[0] as ReadableLogRecord;
            assert.strictEqual(navLogRecord.eventName, EVENT_NAME);
            assert.strictEqual((navLogRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_SAME_DOCUMENT], 
              event.destination?.sameDocument ?? true);
            assert.strictEqual((navLogRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_HASH_CHANGE], 
        ...
        if (navigation.navigate) {
          ...
          setTimeout(() => {
            ...
            if (records.length >= 1) {
              const navLogRecord = records.slice(-1)[0] as ReadableLogRecord;
              assert.strictEqual(navLogRecord.eventName, EVENT_NAME);
              assert.strictEqual((navLogRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_SAME_DOCUMENT], true);
              cleanup();
              done();
            }
          }, 50);
        ...
        if (event.hashChange) {
          setTimeout(() => {
            ...
            if (records.length >= 1) {
              ...
              assert.strictEqual(navLogRecord.eventName, EVENT_NAME);
              assert.strictEqual((navLogRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_SAME_DOCUMENT], true);
              assert.strictEqual((navLogRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_HASH_CHANGE], true);
              assert.strictEqual((navLogRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_HASH_TYPE], 'push');
              cleanup();
              done();
            }
          }, 10);
    ...
    it('should work with Navigation API disabled', done => {
      ...
      setTimeout(() => {
        ...
        assert.strictEqual((testRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_HASH_TYPE], 'push');
        assert.strictEqual((testRecord.attributes as any)[ATTR_BROWSER_NAVIGATION_SAME_DOCUMENT], true);
