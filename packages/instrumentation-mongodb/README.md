...
- [`db.client.connections.usage`](https://github.com/open-telemetry/semantic-conventions/blob/v1.24.0/docs/database/database-metrics.md#metric-dbclientconnectionsusage) - The number of connections currently in a given state.
  ...
  Note: `db.client.connections.usage` is only emitted when the old (pre-stable) database semconv is active i.e. when `OTEL_SEMCONV_STABILITY_OPT_IN` does not include `database`, or includes `database/dup`. It is not emitted when `OTEL_SEMCONV_STABILITY_OPT_IN` includes `database` without `/dup`, since the metric has been deprecated in favor of `db.client.connection.count` in the [semconv database migration](https://opentelemetry.io/docs/specs/semconv/non-normative/db-migration/#database-client-connection-count),and that replacement metric is still unstable. There is ongoing work to provide an opt-in setting to select the latest experimental semconv.
