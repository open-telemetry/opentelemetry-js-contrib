...
function build(platform, arch, target) {
  console.log(`Building: platform: ${platform}, arch: ${arch}, node version: ${target.version}`);
  ...
  fs.readdirSync(folderPath).forEach((folder) => {
    fs.readdirSync(path.join(folderPath, folder))
      ...
      .forEach(file => {
        ...
        const sum = fs.readFileSync(path.join(folderPath, folder, `${file}.sha1`), 'ascii');
        if (sum !== checksum(content)) {
          throw new Error(`Invalid checksum for "${PREBUILDS_FOLDER_NAME}/${folder}/${file}".`);
        }
        filesChecked.push(`${folder}/${file}`);
      });
  });
  if (filesToBeBuild !== filesChecked.length) {
    throw new Error(`Not all files have been checked, files to be build (${filesToBeBuild}), files to be checked (${filesChecked.length})`);
  }
  const time = Math.round((new Date().getTime() - start) / 1000);

  console.log(`All went fine, it took: ${time}s to build all.`);
  console.log(`Number of files generated and checked: (${filesChecked.length}) ->`, filesChecked);
}
