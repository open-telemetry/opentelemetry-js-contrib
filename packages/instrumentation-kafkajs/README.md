...
| `messaging.kafka.cluster.id`         | The Kafka cluster id, read from the broker metadata kafkajs's own producer/consumer already fetch — no separate connection is opened. Best-effort: omitted until that metadata has been fetched (e.g. the first send on a freshly created producer, before kafkajs has connected), or if the broker predates KIP-78 (cluster id support). |
