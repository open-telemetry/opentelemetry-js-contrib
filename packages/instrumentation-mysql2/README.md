...
| `enableTraceContextPropagation`   | `boolean`                                               | If true, injects the active span's W3C `traceparent` into the MySQL session via `SET @traceparent = '...'` before each `query`/`execute` (default false). Unlike SQL Commenter, this also works for prepared statements (`connection.execute`). _NOTE: This adds one extra round-trip per user query._                       |
