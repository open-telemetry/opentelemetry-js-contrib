...
/**
 * Example MCP HTTP Client with OpenTelemetry Instrumentation
 *
...
async function main() {
  ...
  const tracer = trace.getTracer('mcp-http-client');
  const span = tracer.startSpan('mcp-http-client-operation');

  try {
    await context.with(trace.setSpan(context.active(), span), async () => {
      // Create HTTP transport to communicate with server
