...
This module provides automatic instrumentation for the [`memcached@>=2.2.0`][repo-url] module.
...
The `@opentelemetry/instrumentation-memcached` versions 0.63.0 and later emit the stable semantic conventions.
...
| `db.query.text`     | The database statement being executed (only if `enhancedDatabaseReporting` is enabled). |
