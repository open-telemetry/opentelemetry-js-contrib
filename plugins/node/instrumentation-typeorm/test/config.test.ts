...
describe('TypeormInstrumentationConfig', () => {
  ...
  it('enableInternalInstrumentation:true', async () => {
    ...
    instrumentation.setConfig(config);
    const connection = await typeorm.createConnection(defaultOptions);
    const [_users, _count] = await connection.manager.findAndCount(User);
    ...
    expect(findAndCountSpan).toBeDefined();
    expect(findAndCountSpan.attributes[SemanticAttributes.DB_OPERATION]).toBe(
      'findAndCount'
    );
    expect(findAndCountSpan.attributes[SemanticAttributes.DB_SQL_TABLE]).toBe(
      'user'
    );

    const selectSpan = spans.find(
      s => s.name.indexOf('select') !== -1
    ) as ReadableSpan;
    expect(selectSpan).toBeDefined();
    expect(selectSpan.attributes[SemanticAttributes.DB_OPERATION]).toBe(
      'select'
    );
