...
| `blockHeadersToSpanAttributes` | `Object` | List of case insensitive HTTP headers to block from being converted to span attributes. This option takes precedence over `headersToSpanAttributes` if both options are set. In case `headersToSpanAttributes` is not set all headers not present in `blockHeadersToSpanAttributes` will be converted. |
